...
surtiver <- function(z, event, time, strata, spline="Smooth-spline", nsplines=8, ties="none", 
                     control, ...) {
  ...
  if (missing(strata)) strata <- rep(1, length(time))
  
  #order the data:
  order_time <- order(time)
  z <- z[order_time,]
  event <- event[order_time]
  time  <- time[order_time]
  
  p <- ncol(z)
  data <- data.frame(event=event, time=time, z=z, strata=strata, stringsAsFactors=F)
  Z.char <- paste0("z.", 1:p)
...
coxtp.control <- function(tol=1e-9, iter.max=20L, method="ProxN", lambda=1e8,
