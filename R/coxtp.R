...
#' 
#' lambda.spline  = c(0,1)
#' fit   <- coxtp(event = event, z = z, time = time, lambda.spline=lambda.spline)
#' 
...
#' fit a cox Non-proportional Hazards model with P-spline or Smoothing-spline. Information criteria is also provided.
#' 
#' Fit a cox Non-proportional Hazards model via penalized maximum likelihood. 
#' 
#' @details put the link to the wesite()...
...
#'
#' @return
#' \item{model.AIC}{a \code{"coxtp"} object with penalization coefficient chosen based on AIC} 
#' \item{model.TIC}{a \code{"coxtp"} object with penalization coefficient chosen based on TIC}
#' \item{model.GIC}{a \code{"coxtp"} object with penalization coefficient chosen based on GIC}
