...
loessMod10 <- loess(haz_j ~ time_unique, data=smooth_data, span=0.10) # 10% smoothing span
#loessMod30 <- loess(haz_j ~ time_unique, data=smooth_data, span=0.30) # 30% smoothing span
#loessMod50 <- loess(haz_j ~ time_unique, data=smooth_data, span=0.50) # 50% smoothing span

haz_j_smooth10 <- predict(loessMod10) 
...
# colnames(exp_betax) <- colnames(z)

haz_0_betax             <- haz_j_smooth10 * exp_betax
```
...
plot_age <- ggplot(haz_0_betax, aes(x=time_unique)) + 
  geom_line(aes(y=non_cancer,  color = "non-cancer"),size = 0.6) + 
  geom_line(aes(y=non_metastatic,    color = "non-metastatic"),size = 0.6) + 
  geom_line(aes(y=cancer,  color = "metastatic-cancer"),size = 0.6) + 

  # scale_x_continuous(name='Years since diagnosis', limits=c(0,12), breaks=c(0,4,8,12)) + 
  # scale_y_continuous(name='Absolute hazard', limits = c(0,0.2)) +
  ...
  # ggtitle(" Cancer Death Mets Only") +
