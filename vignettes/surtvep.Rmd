...
There were 9104 patients included in the data set.
We study the time-varying effects of age and cancer status as the illustration example.
...
Age and cancer status are included as covariates and considered as categorical variables.
Age has 4 levels and cancer status has 3 levels.
The following codes processed the covariates for model fitting.

```{r, message = FALSE}
#for variable age
age <- cut(support$age, quantile(support$age, c(0,1/4,1/2,3/4,1), na.rm = TRUE), 
     labels = c(0,1,2,3), include.lowest = TRUE)
levels(age) <- c("2","0","1","3")
z_age <- model.matrix(~age)[,-1]
colnames(z_age) <- c("age18_53", "age53_64", "age_74")

# cancer status: metastatic, yes, no
ca <- support$ca
levels(ca) <- c("no", "yes", "metastatic")
z_ca <-  model.matrix(~ca)[,-1]
colnames(z_ca) <- c("non-metastatic", "metastatic")

z <- cbind(z_age, z_ca)
```

<!-- #age variable: treat the youngest age group as the reference group -->
...
<!-- fit <- coxtv(event = death, z = z, time = time) -->

<!-- plot(fit, parm = c("age18_53", "age53_64")) -->

<!-- fit <- coxtp(event = death, z = z, time = time) -->
